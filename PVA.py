...
options = {}
defaults = {
'header': """M98 P"BEGIN.G" ;Begining of Program Macro
""",
'footer': """M98 P"END.G" ;End of Program Macro
"""
}
...
def draw_text(text,x,y, group = None, style = None, font_size = 10, pva_tag = None) :
    if style == None : 
    ...
    if pva_tag!=None : 
        attributes["gcodetools"] = str(pva_tag)
...
class path_gcode(inkex.Effect):
    ...
    def __init__(self):
        ...
        # Build Tab
        self.OptionParser.add_option("",   "--dry-mode",                        action="store", type="inkbool",         dest="dry_mode",                            default="False",                    help="Build a path with no tool on/tool off commands")
        ...
        self.OptionParser.add_option("",   "--layer-travel-speed",              action="store", type="float",           dest="layer_travel_speed",                  default="3000",                     help="Travel speed (mm/min)")
        self.OptionParser.add_option("",   "--layer-safe-z",                    action="store", type="float",           dest="layer_safe_z",                        default="0",                        help="Safe Z height")

        # Path Object Settings Tab
        self.OptionParser.add_option("",   "--po-id",                           action="store", type="string",          dest="po_id",                               default="",                         help="Replaces the id of the selected path object")
        ...
        self.line_settings_field_order = [
                    'Line ID',
                    'Tool',
                    'Dispense Speed',
                    'Valve Off Distance',
        ...
        self.auto_optimize = False  # auto optimize path if true, if false, will go by inverse order of .svg

    def parse_curve(self, p, layer, w = None, f = None, path_type = []):
            c = []
            npt = []
            if len(p)==0 :
                return []

            # we need to call this funtion on ever part of the p list besides circles and ellipses. 
            # need to check if we're working on a dot here. if it is a dot, don't transform_csp
            # fake_p = p
            # j = 0
            # while j < len(p):
                # if 'circle' not in str(path_type[j]) and 'ellipse' not in str(path_type[j]):
                    # fake_p = self.transform_csp(fake_p, layer)
                    # p[j] = fake_p[j]
                # else:
                    # print_("Working on Dot")
            ...
            for k in keys:
                subpath = p[k]
                if 'circle' in str(path_type[k]) or 'ellipse' in str(path_type[k]):
                    c += [ [ [subpath[0][1][0],subpath[0][1][1]], 'dot', 0, 0] ]
                else:
...
#                    print_((-f(w[k][i-1]),-f(w[k][i]), [i1[5] for i1 in l1]) )
                ...
                npt.append(path_type[k])
            # we need to return the newly ordered path as well as the newly ordered path type list
            rl = [c,npt]
            return rl
    ...
    def check_dir(self):
        ...
        if (os.path.isdir(self.options.directory)):
            ...
            self.header += "G21 ;All units in mm\n"
            # elif self.options.unit == "G20 ;All units in inches" :
                # self.header += "G20 ;All units in inches"
        else:
...
###        * adding 'dot' type with an x,y start point
###
################################################################################
    def generate_gcode(self, curve, layer, path):
        global prec
        ...
        j = 0 # path variable. increments when s[1] == 'end'
        for i in range(1,len(curve)):
            # Creating Gcode for curve between s=curve[i-1] and si=curve[i] start at s[0] end at s[4]=si[0]
            s, si = curve[i-1], curve[i]

            ds = " F %f"%float(layer_set['Travel Speed']) if self.is_dot(path[j]) else " F %f"%float(path[j].get('dispense_speed'))
            dispense_speed = ds if lg not in ['G01','G02','G03'] else ''

            print_("path:",j,path[j],path[j].get('id'),", s[1]:",s[1])

            tool  = path[j].get('tool')
            depth = float(path[j].get('dispense_height'))

            if path[j].get('valve_off_distance') is None:
                vod = 0
            else:
                try:
                    vod = float(path[j].get('valve_off_distance'))
                except:
                    vod = 0
    ...
    def transform(self, source_point, layer, reverse=False):
        ...
        if layer not in self.transform_matrix:
            ...
            print_("Layer '%s' transformation matrixes:" % layer.get(inkex.addNS('label','inkscape')) )
...
################################################################################
    def error(self, s, type_= "Warning"):
        ...
        errors = """
                        ...
                        active_layer_already_has_settings
                        apply_on_this_tab_does_nothing
                    """
...
################################################################################
###
###        Set and Display Layer Settings as well as Objects in Layer
###
################################################################################
    def display_layer_settings(self, layer=None):
        ...
        layer_settings = {
            "Layer Name": layer.get(inkex.addNS('label','inkscape')),
            "Travel Speed": layer.get("travel_speed"),
            "Safe Z": layer.get("safe_z"),
        }

        # show error message if layer settings do not exist
        for key in layer_settings:
            if layer_settings[key] == None :
                self.error(_("Layer settings do not exist for layer %s." %layer.get(inkex.addNS('label','inkscape'))),"error")

        # current layer settings are displayed, UPDATE THOSE VALUES
        if ls_element is not None:
            current_position = ls_element.get('transform')
            print_("Current Position:", current_position)
            self.hide_layer_settings(layer)
        ...
        s_rmv = self.remove_spaces(layer_settings['Layer Name']) + '_Layer_Settings'
        layer_group = inkex.etree.SubElement(layer, inkex.addNS('g','svg'), {'id': s_rmv, 'layersettings': "Layer Settings definition"})
        bg = inkex.etree.SubElement(layer_group, inkex.addNS('path','svg'),
            {'style': "fill:#%s;fill-opacity:0.5;stroke:#444444; stroke-width:1px;"%colors[layer_num%len(colors)], "layersettings": "Layer Settings background"})

        # short function to invert color used to highlight selected objects
        def invert_color(color): #        XOR
            color = str(hex(int(color, 16) ^ 0xffffff))[2:]
            while len(color) < 6:
                color = "0" + color
            return color

        y = 0
        tx_style = "font-family:DejaVu Sans;font-style:normal;font-variant:normal;font-weight:normal;font-stretch:normal;font-family:DejaVu Sans;fill:#%s;fill-opacity:1;stroke:none;"%invert_color(colors[layer_num%len(colors)])

        # layer settings
        keys = []
        for key in self.layer_settings_field_order:
            if key in layer_settings: keys += [key]
        for key in layer_settings:
            if key not in keys: keys += [key]
        for key in keys:
            s_rmv = self.remove_spaces(layer_settings['Layer Name']) + '_' + self.remove_spaces(key)
            g = inkex.etree.SubElement(layer_group, inkex.addNS('g','svg'), {'id': s_rmv, 'layersettings': "Layer Settings parameter"})

            draw_text(key, 0, y, group = g, pva_tag = "PVA Layer Settings definition field name", font_size = 10 if key!='Layer Name' else 20)

            param = layer_settings[key]
            if type(param) == str and re.match("^\s*$",param): param = "(None)"

            draw_text(param, 150, y, group = g, pva_tag = "PVA Layer Settings definition field value", font_size = 10 if key!='Layer Name' else 20)

            v = str(param).split("\n")
            y += 15*len(v) if key!='Layer Name' else 20*len(v)

        for l in reversed(layer):

            # line settings
            if 'path' in l.tag:
                y += 10 # space between each line settings group
                ...
                try:
                    set_style = tx_style if l in self.selected_paths[layer] else None
                except:
                    set_style = None

                keys = []
                for key in self.line_settings_field_order:
                    if key in line_settings: keys += [key]
                for key in line_settings:
                    if key not in keys: keys += [key]
                for key in keys:
                    s_rmv = self.remove_spaces(line_settings['Line ID']) + '_' + self.remove_spaces(key)
                    g = inkex.etree.SubElement(layer_group, inkex.addNS('g','svg'), {'id': s_rmv, 'linesettings': "Line Settings parameter"})

                    draw_text(key, 0, y, group = g, style = set_style, pva_tag = "PVA Line Settings definition field name", font_size = 10 if key!='Line ID' else 12)

                    param = line_settings[key]
                    if type(param) == str and re.match("^\s*$",param): param = "(None)"

                    draw_text(param, 150, y, group = g, style = set_style, pva_tag = "PVA Line Settings definition field value", font_size = 10 if key!='Line ID' else 12)

                    v = str(param).split("\n")
                    y += 15*len(v) if key!='Layer Name' else 20*len(v)

            # dot settings
            elif 'circle' in l.tag or 'ellipse' in l.tag:
                y += 10 # space between each dot settings group
                ...
                keys = []
                for key in self.dot_settings_field_order:
                    if key in dot_settings: keys += [key]
                for key in dot_settings:
                    if key not in keys: keys += [key]
                for key in keys:
                    s_rmv = self.remove_spaces(dot_settings['Dot ID']) + '_' + self.remove_spaces(key)
                    g = inkex.etree.SubElement(layer_group, inkex.addNS('g','svg'), {'id': s_rmv, 'dotsettings': "Dot Settings parameter"})

                    draw_text(key, 0, y, group = g, style = set_style, pva_tag = "PVA Dot Settings definition field name", font_size = 10 if key!='Dot ID' else 12)

                    param = dot_settings[key]
                    if type(param) == str and re.match("^\s*$",param): param = "(None)"

                    draw_text(param, 150, y, group = g, style = set_style, pva_tag = "PVA Dot Settings definition field value", font_size = 10 if key!='Dot ID' else 12)

                    v = str(param).split("\n")
                    y += 15*len(v) if key!='Layer Name' else 20*len(v)

        bg.set('d',"m -20,-20 l 400,0 0,%f -400,0 z " % (y+50))
        # layer_group.set("transform", simpletransform.formatTransform([ [1,0,self.view_center[0] ], [0,1,self.view_center[1]] ] ))
        layer_group.set("transform", simpletransform.formatTransform([ [1,0,(layer_num-3) * 400 ], [0,1,350] ] ) if current_position == '' else current_position)
...
###
################################################################################
    def hide_layer_settings(self, layer=None):
        layer = self.current_layer if self.current_layer is not None or layer == None else self.document.getroot()
        ls_element = self.getElementById(self.remove_spaces(layer.get(inkex.addNS('label','inkscape'))) + '_' + 'Layer_Settings')

        if ls_element is not None:
            print_("Remove:",ls_element.get("id"),", From:",layer.get("id"))
            layer.remove(ls_element)


################################################################################
###
###        Import Layer Settings From Displayed Layer Settings
###
################################################################################
    def import_layer_settings(self, layer=None):
        ...
        if ls_element == None:
            self.error(_("Layer settings are not displayed for layer %s." %layer.get(inkex.addNS('label','inkscape'))),"error")

        # loop through the individual object settings and apply them as we go
        i = 0
        for sub_e in ls_element:
            if 'path' in sub_e.tag:
                i += 1
                continue

            if sub_e[0][0].text == 'Layer Name' or sub_e[0][0].text == 'Dot ID' or sub_e[0][0].text == 'Line ID':
                j = 1
                e = layer if sub_e[0][0].text == 'Layer Name' else self.getElementById(ls_element[i][1][0].text)
                t = 3 if sub_e[0][0].text == 'Layer Name' else (4 + (sub_e[0][0].text == 'Line ID'))

                while j < t:
                    print_("In", e, "Setting:",self.remove_spaces(ls_element[i+j][0][0].text.lower()), "To:",ls_element[i+j][1][0].text)
                    e.set(self.remove_spaces(ls_element[i+j][0][0].text.lower()), ls_element[i+j][1][0].text)
                    j += 1
...
################################################################################
###
###        Set and Display Layer Settings
###
################################################################################
    def set_layer_settings(self, layer=None):
        if layer is None:
            layer = self.current_layer

        global prec

        print_("Setting Travel Speed:", str(round(self.options.layer_travel_speed,prec)), "and Safe Z:", str(self.options.layer_safe_z), "for Layer:", layer.get(inkex.addNS('label','inkscape')))

        layer.set('travel_speed', str(round(self.options.layer_travel_speed,prec)))  # set travel speed
        layer.set('safe_z', str(round(self.options.layer_safe_z,prec)))              # set safe z


################################################################################
###
###        Gets Layer Setting to Transfer to gcode
###
################################################################################
    def get_layer_settings(self, layer=None):
        if layer is None:
            layer = self.current_layer
...
################################################################################
###
###        Set Path Object(s) Settings
###
################################################################################
    def set_path_settings(self, selected_objects):
        new_id = self.options.po_id
        global prec

        # error if no line/curve objects are selected
        if selected_objects == {}:
            self.error(_("No path objects selected. Please select a path object to apply dispense settings."),"error")

        # error if tool field is blank
        if self.options.tool == '':
            self.error(_("Tool field cannot be blank"),"error")

        i = 1
        for key in selected_objects:
            for path_object in selected_objects[key]:

                # set the line/curve object id if the new id is not blank
                # if multiple selected, add numeric suffix
                if new_id != '':
                    if len(selected_objects[key]) == 1 and len(selected_objects) == 1:
                        print_(self.getElementById(new_id))
                        print_(path_object)
                        if self.getElementById(new_id) is not None and self.getElementById(new_id) is not path_object:
                            self.error(_("ID already exists. ID's must be unique"),"error")
                        else:
                            path_object.set('id', new_id)
                    else:
                        if self.getElementById(new_id + '_' + str(i)) is not None and self.getElementById(new_id + '_' + str(i)) is not path_object:
                            self.error(_("ID already exists. ID's must be unique"),"error")
                        else:
                            path_object.set('id', new_id + '_' + str(i))
                    i += 1

                # settings for both lines and dots
                path_object.set('tool', str(self.options.tool))                       # set tool
                path_object.set('dispense_height', str(round(self.options.dispense_height,prec))) # set dispense height
...
################################################################################
###
###        Get Path Object Settings
###
################################################################################
    def get_path_settings(self, po):
        # po is a circle or ellipse. will apply dispense dot settings
        if self.is_dot(po):
            po_settings = {
                'ID': po.get('id'),
                'Tool': po.get('tool'),
...
################################################################################
###        Test if Object is a Circle/Ellipse or Path
################################################################################
    def is_dot(self, path_object):
        try: 
            dot = 'circle' in path_object.tag or 'ellipse' in path_object.tag
        except:
            dot = False
        return dot
    ...
    def get_info(self):
        ...
        def recursive_search(g, layer, selected=False):
            ...
            for i in items:
                ...
                elif i.tag == inkex.addNS('path','svg') or i.tag == inkex.addNS('circle','svg') or i.tag == inkex.addNS('ellipse','svg'):
                ...
                elif i.get("id") in self.selected and self.options.active_tab == 'build':
...
################################################################################
    def create_path(self) :
        ...
        # throw an error if no paths were selected
        if self.selected_paths == {}:
            paths=self.paths
            self.error(_("No paths are selected. Please select the paths you want to build."),"error")
        else :
        ...
        biarc_group = inkex.etree.SubElement( self.selected_paths.keys()[0] if len(self.selected_paths.keys())>0 else self.layers[0], inkex.addNS('g','svg'), {"id":"Begin_Paths"})

        for layer in self.layers :
            print_("Layer Name: %s" % (layer.get(inkex.addNS('label','inkscape'))))
            if layer in paths :
                p = []
                p_path = []
                dxfpoints = []
                for path in paths[layer] :
                    ...
                    # the selected object does not have settings
                    if path.get('dispense_height') == None:
                        self.error(_("There are objects you have selected that do not have settings. Please apply settings to all objects and layers before building the path."),"error")

                    if "d" not in path.keys():
                        if self.is_dot(path):
                            # need to add the 'd' command to the circle element or fake the 'd'
                            path.set('d','M ' + str(path.get('cx')) + ' ' + str(path.get('cy')))
                        else:
                ...
                rl = self.parse_curve(p, layer, path_type = p_path)
                curve = rl[0]
                p_path = rl[1]
                self.draw_curve(curve, layer, biarc_group)
                gcode += ";Layer: " + str(layer.get(inkex.addNS('label','inkscape'))) + "\n"
                gcode += self.generate_gcode(curve, layer, p_path)
...
################################################################################
    def delete_paths(self) :
        if self.getElementById('Begin_Paths') is not None:
            self.getParentNode(self.getElementById('Begin_Paths')).remove(self.getElementById('Begin_Paths'))
...
################################################################################
    def effect(self) :
        ...
        if self.options.active_tab == '"build"':

            self.delete_paths()
            self.create_path()
            self.delete_paths()

        elif self.options.active_tab == '"layer-settings"':
            if self.options.layer_action == "layer-set":
                self.set_layer_settings()
                print_("Layer Settings:",self.get_layer_settings())

            elif self.options.layer_action == "layer-display":
                self.display_layer_settings()

            elif self.options.layer_action == "layer-hide":
                self.hide_layer_settings()

            elif self.options.layer_action == "layer-import":
                self.import_layer_settings()

        elif self.options.active_tab == '"path-settings"':

            # set selected dot settings
            self.set_path_settings(self.selected_paths)

            # get selected dot settings
            for key in self.selected_paths:
                for path_object in self.selected_paths[key]:
                    if self.is_dot(path_object):
                        print_("Dot:", self.get_path_settings(path_object))
                    else:
                        print_("Line:", self.get_path_settings(path_object))

        else:
            self.error(_("Pressing 'Apply' on this tab does nothing"),"apply_on_this_tab_does_nothing")
